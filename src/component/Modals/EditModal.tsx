...
const EditInvoiceModal: React.FC<EditInvoiceModalProps> = ({ isOpen, onClose, invoice }) => {
  ...
  const overallTotal = useMemo(() => formData.items.reduce((sum, item) => sum + (item.total || 0), 0), [formData.items])
  ...
  return (
    ...
    >
      ...
      <div
        ref={modalRef}
        className={`
          relative h-full pl-40 flex flex-col bg-white dark:bg-[#1E2139]
